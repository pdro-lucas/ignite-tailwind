...
const fileItem = tv({
  ...
  variants: {
    state: {
      ...
      complete: {
        container: "bg-violet-25 border-violet-500",
      },
      error: {
    ...
    <div className={container()}>
      ...
      {state === "error" ? (
        <div className="flex flex-1 flex-col items-start gap-1">
          ...
          <button className="text-sm font-semibold text-error-700 underline hover:text-error-900">
            Try again
          </button>
        </div>
      ) : (
        <div className="flex flex-1 flex-col items-start gap-1">
          <div className="flex flex-col">
            <span className="text-sm font-medium tex-zinc-700">{name}</span>
